...
export async function POST(req: Request) {
  ...
  if (action === 'buy') {
    ...
    transaction = {
      ...
      amount: -quotedAmount,
      fee,
      description: `${liveAsset.symbol} Buy (${liveAsset.network}) — ${formatCrypto(cryptoAmount, liveAsset.symbol)}`,
      reference: ref,
    ...
    transaction = {
      ...
      amount: netCredit,
      fee,
      description: `${liveAsset.symbol} Sell (${liveAsset.network}) — ${formatCrypto(cryptoAmount, liveAsset.symbol)}`,
      reference: ref,
      createdAt: new Date().toISOString(),
      icon: '₿',
      metadata: {
